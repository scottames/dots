...
`@narumitw/pi-statusline@0.16.0`, and `pi-subagents@0.34.0` on the current Linux
host with Pi 0.80.7, `pi-local.json`, and the `nono-pi` launcher. It accepts
the expected terminal-rendering risk and that `pi-subagents` and its children
inherit Pi's environment and Nono-authorized authority. The installer is
portable to macOS, but macOS requires a separate review and approval before
activation. Re-review after changing any package, Pi, operating system, Nono
profile, or launcher.
...
expected dependency set in `scripts/test-pi-extension-supply-chain.sh`; then
perform the update checklist and record a new approval.
